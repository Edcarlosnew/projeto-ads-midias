...
// --- ROTA DE TRANSCRIÇÃO (MODIFICADA PARA SIMULAÇÃO DINÂMICA) ---
...
  try {
    // --- PASSO ADICIONADO: Buscar o título da mídia ---
    const [rows] = await db.query('SELECT titulo FROM midias WHERE id = ?', [id]);
    
    if (rows.length === 0) {
      console.log(`Mídia com ID ${id} não encontrada.`);
      return res.status(404).json({ error: 'Mídia não encontrada.' });
    }
    
    const titulo = rows[0].titulo;
    console.log(`Gerando simulação para o título: ${titulo}`);

    // --- MODO DE SIMULAÇÃO (DINÂMICO COM OS TÍTULOS MAIS RECENTES) ---
    // Usamos o 'switch' com os títulos exatos da sua última imagem
    switch (titulo) {
      ...
      case "Miami Sound Machine - Bad Boy (official video reworked)":
        ...
        break;
      default:
        // Um texto padrão caso o título não seja reconhecido
