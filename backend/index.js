...
app.post('/midias', async (req, res) => {
  ...
  try {
    ...
    const novaMidia = {
      ...
      url_midia: url_midia,
      usuario_id: usuarioId,
      texto_transcricao: null // Adiciona o campo para consistência
    };
    res.status(201).json(novaMidia);
  } catch (error) {
    console.error('Erro ao criar mídia:', error);
...
// --- ROTA DE TRANSCRIÇÃO (VERSÃO FINAL COM OPENAI) ---
app.post('/midias/:id/transcrever', async (req, res) => {
  const { id } = req.params;
  console.log(`Recebido pedido para transcrever a mídia com ID: ${id}`);

  try {
    // 1. Define o caminho para o nosso ficheiro de áudio de teste
    const audioPath = path.join(__dirname, 'uploads', 'sample.mp3');

    if (!fs.existsSync(audioPath)) {
      console.error('Ficheiro de áudio de teste (sample.mp3) não encontrado na pasta uploads!');
      return res.status(404).json({ error: 'Ficheiro de áudio de teste não encontrado.' });
    }

    console.log(`A enviar o ficheiro de áudio para a OpenAI: ${audioPath}`);

    // 2. Enviar o ficheiro de áudio para a API do Whisper
    const transcription = await openai.audio.transcriptions.create({
      file: fs.createReadStream(audioPath), // Lê o nosso ficheiro de áudio
      model: "whisper-1",                   // Especifica o modelo de IA a ser usado
    });

    // A API devolve o texto transcrito
    const textoTranscribed = transcription.text;
    console.log('Texto recebido da OpenAI:', textoTranscribed);

    // 3. Guardar o texto transcrito no nosso banco de dados
    await db.query(
      'UPDATE midias SET texto_transcricao = ? WHERE id = ?',
      [textoTranscribed, id]
    );
